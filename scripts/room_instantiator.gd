...
func instantiate_rooms(rooms_created):
	for room_cords in rooms_created:
		...
		add_child(temp_room)
		temp_room.position = Vector2(room_cords.x * room_width, room_cords.y * room_height) 
		_make_doors(room_cords, temp_room)

func _make_doors(cords, temp_room):
	if room_generator._is_room_existing(Vector2(cords.x + 1, cords.y)):
	...
	if room_generator._is_room_existing(Vector2(cords.x, cords.y - 1)):
